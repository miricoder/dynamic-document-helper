...
def scrape_and_ingest_data(
    website_link: str, api_key: str, pinecone_api_key: str, pinecone_environment: str
):
...
def run_chat(
    prompt: str, api_key: str, pinecone_api_key: str, pinecone_environment: str
):
...
async def main():
    ...
    pinecone_environment = st.sidebar.text_input(
        "Enter Pinecone Environment Region:", type="password"
    )
    ...
    if st.session_state.get("api_key_validated", False):
        ...
        if st.session_state.get("chat_answer_history"):
            ...
            for i, (generated_response, user_query) in enumerate(reversed(history_zip)):
                message(
                    user_query, is_user=True, key=f"user_{len(history_zip) - i - 1}"
                )
